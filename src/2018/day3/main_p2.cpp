...
int main(int argc, char **argv) {
  ...
  std::map<std::pair<int, int>, std::vector<int>> postoid;
  std::map<int, bool> idtooverlap;
  while (std::getline(file, line)) {
    ...
    while (it != end) {
      ...
      ++it;
      idtooverlap[current.id] = false;
    }
    for (int i = current.y; i < current.y + current.height; i++) {
      for (int j = current.x; j < current.x + current.width; j++) {
        fabric[i][j]++;
        postoid[{i, j}].push_back(current.id);
        if (postoid[{i, j}].size() > 1) {
          for (int k = 0; k < postoid[{i, j}].size(); k++) {
            int id = postoid[{i, j}][k];
            idtooverlap[id] = true;
          }
        }
      }
  ...
  file.close();
  for (int i = 1; i <= idtooverlap.size(); i++) {
    if (idtooverlap.at(i) == false) {
      std::cout << i << '\n';
    }
  }
  return 0;
