...
struct pos
{
  ...
  pos (int x_, int y_, std::pair<int, int> direction_, int sign_)
      : x (x_), y (y_), direction (std::move (direction_)), signature (sign_)
  {
  }
  void
  change_direction (const std::pair<int, int> &new_dir)
  {
...
struct mirror
{
  ...
  std::pair<int, int> TO;
  mirror (int x_, int y_, std::pair<int, int> FROM_, std::pair<int, int> TO_)
      : x (x_), y (y_), FROM (std::move (FROM_)), TO (std::move (TO_))
  {
  }
  bool
  operator<(const mirror &other) const
  {
          ...
          for (auto &beam : beams)
            {
              ...
              if (map[beam.x][beam.y] == '/')
                {
                  mirror cur_mir{ beam.x, beam.y, beam.direction,
                                  std::make_pair (-beam.direction.second,
                                                  -beam.direction.first) };
                  ...
                  beam.change_direction (std::make_pair (
                      -beam.direction.second, -beam.direction.first));
                }
              ...
              else if (map[beam.x][beam.y] == '|')
                {
                  ...
                  local_beams.emplace_back (
                      beam.x, beam.y,
                      std::make_pair (-beam.direction.first,
                                      -beam.direction.second),
                      1);
                }
              else if (map[beam.x][beam.y] == '-')
                {
