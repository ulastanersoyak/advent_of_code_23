...
std::vector<char>
get_adj_tiles (const std::vector<std::string> &tilemap, const position &start)
{
  std::vector<char> res;
...
char
get_start_tile (const std::vector<char> &adjacent_tiles)
{
  std::vector<std::pair<position, position> > directions;
  directions.reserve (adjacent_tiles.size ());
...
int
traverse (std::vector<std::string> &tilemap, rat &player)
{
          ...
          if (tilemap[i][j] != 'S')
            {
              ...
              std::string right
                  = tilemap[i].substr (j, tilemap[i].size () - j);
