...
struct disc {
  ...
  bool operator<(const disc &other) const { return name < other.name; }
};

struct disc parse_line(const std::string &arg) {
  size_t open_paranthesis = arg.find('(');
  std::string name = arg.substr(0, arg.find(' '));
  int weight = std::stoi(
      arg.substr(open_paranthesis + 1, arg.find(')') - open_paranthesis - 1));
  ...
  if (arrow != std::string::npos) {
    ...
    while (std::getline(iss, element, ',')) {
      ...
      element.erase(std::find_if(element.rbegin(), element.rend(),
                                 [](int ch) { return !std::isspace(ch); })
                        .base(),
                    element.end());
...
int traverse(const std::map<std::string, disc> &discmap, const disc &head) {
  if (head.name == "gtervu") {
    int x;
    x++;
  }
