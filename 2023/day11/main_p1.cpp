...
int main(int argc, char **argv) {
  ...
  for (int i = 0; std::getline(file >> std::ws, line); i++) {
    if (line.find('#') == std::string::npos) {
      ...
      for (int j = 0; j < line.size(); j++) {
        if (line[j] == '#') {
          galaxy_positions.push_back(std::pair<int, int>(i, j));
        }
      }
    }
  ...
  for (int i = 0; i < galaxy_positions.size(); i++) {
    ...
    for (int j = i + 1; j < galaxy_positions.size(); j++) {
      ...
      int x_distance = std::abs(current_galaxy.first - target_galaxy.first);
      int y_distance = current_galaxy.second - target_galaxy.second;
      int rs = 0;
      for (int x_step = 0; x_step < x_distance; x_step++) {
        rs += (universe[current_galaxy.first + x_step][current_galaxy.second] ==
               '@')
                  ? 2
                  : 1;
      }
      if (y_distance < 0) {
        ...
        for (int y_step = 1; y_step <= y_distance; y_step++) {
          rs += (universe[target_galaxy.first][target_galaxy.second + y_step] ==
                 '@')
                    ? 2
                    : 1;
        }
